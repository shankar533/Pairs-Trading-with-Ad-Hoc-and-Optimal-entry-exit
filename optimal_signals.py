...
class SleepyYellowGreenFrog(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.risk_free_rate = 0.066
        self.optimizer_options = {
            'maxiter': 1000,  # Increase maximum number of iterations
            'fatol': 1e-2,   # Set function tolerance to a higher value
        }
    ...
    def OnSecuritiesChanged(self, changes):
        ...
        self.high_coint_pairs = self.FindCointegratedPairs(changes.AddedSecurities)
    ...
    def FindCointegratedPairs(self, securities):
        ...
        return [pair[0] for pair in high_coint_pairs]
    def Rebalance(self):
        ...
        if high_var_model is not None:
            ...
            self.TradingStrategyWithoptimalbands(self.high_coint_pairs,high_var_model, high_var_model.endog, -1, 1)
    ...
    def TradingStrategyWithoptimalbands(self,high_pairs,var_model, coint_factor, lower_band, upper_band):
        # Retrieve the mid-prices for the selected pairs
        ...
        for pair in high_pairs:
            ...
            cointegration_strength = coint_factor[-1].mean()
            entry, exit = self.calculate_optimal_boundaries(spread_mean, spread_std, cointegration_strength)
            # Example logic for entry and exit
            if  (coint_factor[-1] < entry).any():
                self.SetHoldings(pair[0], -0.075/len(high_pairs))  # Long
                self.SetHoldings(pair[1],0.05* var_model.coefs[:, 1].sum() / len(high_pairs))  # Exit long
                self.tradedPairs.add((min(pair[0], pair[1]), max(pair[0], pair[1])))
            elif (coint_factor[-1] > exit).any():
                self.SetHoldings(pair[0], 0.075/len(high_pairs))  # Long
                self.SetHoldings(pair[1],-0.05* var_model.coefs[:, 1].sum() / len(high_pairs))  # Exit long
                self.tradedPairs.add((min(pair[0], pair[1]), max(pair[0], pair[1])))
            else:
    ...
    def calculate_optimal_boundaries(self, spread_mean, spread_std, cointegration_strength):
        """Calculate optimal entry and exit boundaries based on a simple PDE model."""
        def objective_function(x):
            entry_boundary, exit_boundary = x

            def pde_objective(z):
                r = self.risk_free_rate
                rho = cointegration_strength
                return (r * z - 0.5 * spread_std**2 * z**2) * (1 - rho**2) - r * z

            entry_integral = quad(pde_objective, -np.inf, entry_boundary)[0]
            exit_integral = quad(pde_objective, exit_boundary, np.inf)[0]

            return (abs(entry_integral) - abs(exit_integral))**2

        initial_guess = [spread_mean - spread_std, spread_mean + spread_std]
        result = minimize(objective_function, initial_guess, method='Nelder-Mead',options=self.optimizer_options)

        if result.success:
            return result.x
        else:
            self.Log("Optimization did not converge. Last result: " + str(result))
            return initial_guess  # Return initial guess as a fallback
